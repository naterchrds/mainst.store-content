					...
					<?php
						...
						if ( ! isset( $matches_emb[0] ) && isset( $matches_sou[0] )) {
							...
							foreach ($match as $matc) {
								if (isset( $matc[0] )) {
									if (wp_oembed_get($matc[0]) && $matc[0] != 1) {
										echo '<div class="col-xs-12 c-entry-video"><div class="c-video-embed">' . wp_oembed_get($matc[0]) . '</div></div>';
									}
								}
							}
						}
                ...
                <div class="item-content">
                    <?php
						...
						foreach ( $match[0] as $amatch ) {
							if ( strpos($amatch, 'youtube.com' ) !== false || strpos( $amatch, 'youtu.be' ) !== false || strpos( $amatch, 'vimeo.com' ) !== false || strpos( $amatch, 'wordpress.tv' ) !== false || strpos( $amatch, 'videopress.com' ) !== false ) {
