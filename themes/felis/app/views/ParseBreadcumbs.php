<?php
    ...
    class ParseBreadcumbs
    {
        ...
        public function render($echo = 1)
        {
            ...
            $text['home']     = esc_html__('Home', 'felis'); // text for the 'Home' link
            $text['category'] = '%s'; // text for a category page
            $text['search']   = esc_html__('Search Results for', 'felis') . ' "%s"'; // text for a search results page
            $text['tag']      = esc_html__('Tag', 'felis') . ' "%s"'; // text for a tag page
            $text['author']   = ' %s'; // text for an author page
            $text['404']      = esc_html__('404', 'felis'); // text for the 404 page
            ...
            if ( is_front_page() ) {
                ...
                if ($show_on_home == 1) {
                    $html .= '<div class="c-page-breadcrumb"><a href="' . esc_url( $home_link ) . '">' . $text['home'] . '</a></div>';
                }

            } elseif ( is_home() ) {
                $title = get_option('page_for_posts') ? get_the_title(get_option('page_for_posts')) : esc_html__('Blog', 'felis');
                $html .= '<div class="c-page-breadcrumb"><a href="' . esc_url( $home_link ) . '">' . $text['home'] . '</a> / ' . $title . '</div>';
            } else {
                ...
                if ($show_home_link == 1) {
                    if ( function_exists( 'is_shop' ) && is_shop() || function_exists( 'is_product' ) && is_product() ) {
                        ...
                        $html .= '<a href="' . esc_url( $home_link ) . '">' . $text['home'] . '</a>'; //rel="v:url" property="v:title"
