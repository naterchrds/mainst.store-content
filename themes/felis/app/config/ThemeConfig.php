...
class ThemeConfig {
    ...
    public static function getRequiredPlugins(){
        if(file_exists(get_parent_theme_file_path('/app/config-required-plugins.php'))){
            include get_parent_theme_file_path('/app/config-required-plugins.php');
            
            return $cactus_required_plugins;
        }
	...
	public static function getAllThumbSizes(){
		if(file_exists(get_parent_theme_file_path('/app/config-image-sizes.php'))){
            include get_parent_theme_file_path('/app/config-image-sizes.php');
            
            return apply_filters('felis_thumbnail_sizes', $cactus_image_sizes);
        }
	...
	public static function thumbSizeMapping($preferred_size){
        if(file_exists(get_parent_theme_file_path('/app/config-image-sizes.php'))){
            ...
            $mapping = apply_filters('felis_thumbnail_sizes', $cactus_image_size_mapping);
	...
	public function getConfiguredThumbSizes(){
		...
		$vals = apply_filters('cactus_thumb_config', $availabe_sizes);
		
		return $vals;
	}
    
    public function __construct(){
        add_action( 'cactus_reg_thumbnail', array($this, 'register_thumbsizes'), 1, 1 );
    }
