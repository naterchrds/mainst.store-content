...
class CT_Shortcodes {
		...
		if(is_singular()){
			...
			if(isset($shortcodes) && is_array($shortcodes) && count($shortcodes) > 0){
				...
				foreach($shortcodes as $tag){
					...
					$config = cactus_get_shortcode_config($tag);
	...
	function enqueue_scripts(){
		...
		wp_enqueue_script( 'ct-shortcode-js', CT_SHORTCODE_PLUGIN_URL . 'shortcodes/js/felis-shortcodes.js', array('imagesloaded', 'slick'), CT_SHORTCODE_VERSION, true );
	}
	...
	function register_imagesizes(){
		...
		global $cactus_size_array; // defined in CactusThemes's theme
		...
		$cactus_size_array_shortcode = apply_filters('cactus_shortcode_image_sizes', $cactus_size_array_shortcode);
 ...
 *
 */
if(!function_exists('cactus_remove_wpautop')){    

    function cactus_remove_wpautop( $content, $autop = false ) {
 ...
 */
add_filter( 'the_content', 'cactus_remove_unwanted_p', 11 );

if(!function_exists('cactus_remove_unwanted_p')){
    function cactus_remove_unwanted_p( $content = null ) {
        if ( $content ) {
            $s = array(
                '/' . preg_quote( '</div>', '/' ) . '[\s\n\f]*' . preg_quote( '</p>', '/' ) . '/i',
                '/' . preg_quote( '<p>', '/' ) . '[\s\n\f]*' . preg_quote( '<div ', '/' ) . '/i',
 ...
 */
function cactus_get_shortcode_config($tag){
	global $cactus_shortcodes;
	foreach($cactus_shortcodes as $name => $params ){
		if(isset($params['tag'])){
			if( $tag == $params['tag'] ){
...
if(!function_exists('cactus_shortcode_get_category')){
    function cactus_shortcode_get_category( $cats_class = 0 ) {
		$class = '';
		if($cats_class == 1){
		...
		foreach ( $categories as $cat ) {
			...
			echo $separate.'<a href="' . esc_url( $cat_url ) . '" class="item-cat '.$class.'" title="' . esc_html__( 'View all posts in ', 'felis' ) . esc_attr( $cat_name ) . '">' . esc_html( $cat_name ). '</a>';
		}
	}
}

if(!function_exists('cactus_shortcode_metadatas')){
    function cactus_shortcode_metadatas( $only_cats = 0, $remove_cats = 0 ) {
	?>
   		<div class="item-meta">
			<ul>
				<?php if ($only_cats != 1) :?>
				...
				<li>
					...
					<?php echo esc_html__('By', 'felis'). ' '. get_the_author(); ?>
				</a>
				</li>
				...
				<li>
					<?php cactus_shortcode_get_category();?>
				</li>
				<?php endif;?>
