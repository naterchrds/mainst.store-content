...
class FW_Extension_Backups_Demo extends FW_Extension {
	...
	protected function _init() {
		...
		// cactus welcome page hook
		add_action( 'cactus_welcome_importdata_tab_content', array($this, '_display_page') );
		// -------------------
	...
	private function is_page() {
		return ( (($current_screen = get_current_screen()) && $current_screen->id === 'tools_page_'. $this->get_page_slug()) || (($current_screen = get_current_screen()) && $current_screen->id === 'toplevel_page_cactus-welcome' ) );
	...
	public function do_install(FW_Ext_Backups_Demo $demo) {
		...
		do_action( 'cactus_backup_before_install_demo', $demo, $collection, $id_prefix, $tmp_dir );
