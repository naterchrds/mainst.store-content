...
class FW_Extension_Backups extends FW_Extension {
	...
	public function _action_enqueue_scripts() {
		if ($this->is_backups_page()) {
			...
			// cactus custom create demo localize
			wp_localize_script(
				'fw-ext-backups',
	...
	public function _action_ajax_delete() {
		...
		$downloadPath = CACTUS_UNYSON_BACKUP_DIR.'/theme-demo/'. $text_domain . '/' . $folder_name;
	...
	public function _action_ajax_create() {
		...
		if ( $archives ) {
			...
			$res = $zip->open( $path );
			if ($res === TRUE) {
			  $zip->extractTo( CACTUS_UNYSON_BACKUP_DIR.'/theme-demo/'. $text_domain . '/' . $folder_name . '/' );
			  $zip->close();
			}
			...
			$content = fopen( CACTUS_UNYSON_BACKUP_DIR.'/theme-demo/'. $text_domain . '/' . $folder_name . '/manifest.php', 'w') or die("Unable to open file!");
	...
	function _action_ajax_create_remote() {
		...
		if ( $archives ) {
			...
			$text_domain = $current_theme->get('TextDomain');
			$path = $archives[ $filename ][ 'path' ];
			$dstfile = CACTUS_UNYSON_BACKUP_DIR.'/remote-demo/'. $text_domain . ''.$filename;
			mkdir( dirname($dstfile), 0777, true );
			copy( $path, $dstfile);

			// create config file
			// $config = $this->_prepare_config( $filename, $dstfile );
			// $content = fopen( CACTUS_UNYSON_BACKUP_DIR.'/remote-demo/'. $text_domain . '/config.php', 'w') or die("Unable to open file!");
			// fwrite( $content, $config );
			// fclose( $content );
			// create script file

			if ( !file_exists( CACTUS_UNYSON_BACKUP_DIR.'/remote-demo/'. $text_domain . '/script.php' ) ) {
				$script = $this->_prepare_script();
				$content = fopen( CACTUS_UNYSON_BACKUP_DIR.'/remote-demo/'. $text_domain . '/script.php', 'w') or die("Unable to open file!");
	...
	public function _action_download() {
		if ( isset( $_GET[self::$download_GET_demo_parameter] ) ) {
			...
			$rootPath = realpath( CACTUS_UNYSON_BACKUP_DIR.'/theme-demo/'. $text_domain . '/' . $folder_name . '/' );

			$download_zip->open( CACTUS_UNYSON_BACKUP_DIR.'/theme-demo/'. $text_domain . '/'. $demo_name , ZipArchive::CREATE | ZipArchive::OVERWRITE);
			...
			$downloadPath = realpath( CACTUS_UNYSON_BACKUP_DIR.'/theme-demo/'. $text_domain . '/' );
