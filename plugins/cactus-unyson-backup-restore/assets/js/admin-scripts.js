jQuery(function($){
	$( document ).ajaxComplete(function( event, request, settings ) {
		if ( 'action=fw:ext:backups:status' == settings.data || 'action=fw%3Aext%3Abackups%3Astatus' == settings.data ) {
			...
			$.each( rows, function(i,e){
				...
				innerHTML += ' | <a href="#" data-package="'+cactusBackup.upload_base_url+'/fw-backup/'+downloadFile+'" class="cactus-create-demo" id="create-'+id+'" data-confirm="Warning! You are about to create a demo content from this backup. Are you sure?">Create Demo</a>';
				$(e).html( innerHTML );
			} );
		}
	});
	...
	$(document).on( 'click', '.cactus-create-demo', function(e){
		e.preventDefault();
		var package = $(this).data('package');
		var packageID = $(this).attr('id');
		packageID = packageID.replace( 'create-', '' );
		$.ajax({
			url: cactusBackup.ajax_url,
			type: 'POST',
			dataType: 'json',
			data: {
				action  : 'cactus-create-demo',
				package : package,
				packageID : packageID,
			},
